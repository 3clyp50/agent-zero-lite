# Development manual for Agent Zero Lite
This guide will show you how to setup a local development environment for Agent Zero Lite in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Agent Zero Lite runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Agent Zero Lite is available on GitHub [github.com/agent0ai/agent-zero-lite](https://github.com/agent0ai/agent-zero-lite).
- You can download the files using a browser and extract or run `git clone https://github.com/agent0ai/agent-zero-lite` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/agent0ai/agent-zero-lite`, so my project folder is `~/Desktop/agent-zero`.
...
- Agent Zero Lite comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.
...
2. Prepare the python environment to run Agent Zero Lite in. (⚠️ This step assumes you have some Python runtime installed.) By clicking the python version in lower right corner (3.13.1 in my example), you should get a list of available environments. You can click the `+ Create Virtual Environment` button. You might be prompted to select the environment manager if you have multiple installed. I have venv and Conda, I will select Conda here. I'm also prompted for desired python version, I will select 3.12, that is known to work well.
...
```
Install `requirements.voice.txt` only if you plan to enable Whisper or Kokoro locally. Install `requirements.local-embeddings.txt` only if you plan to use local HuggingFace embeddings.
Errors in the code editor caused by missing packages should now be gone. If not, try reloading the window.


## Step 4: Run Agent Zero Lite in the IDE
Great work! Now you should be able to run Agent Zero Lite from your IDE including real-time debugging.
...
After inserting my API key in settings, my Agent Zero Lite instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Agent Zero Lite in Docker
...
1. Pull the docker image `agent0ai/agent-zero-lite` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
Agent Zero Lite runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running Agent Zero Lite container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete Agent Zero Lite development environment. You now have:
...
- The ability to develop, debug, and test Agent Zero Lite features seamlessly

You're now ready to contribute to Agent Zero Lite, create custom extensions, or modify the framework to suit your needs. Happy coding!
