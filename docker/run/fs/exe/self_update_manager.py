...
OFFICIAL_REPO_URL = os.environ.get(
    ...
    "https://github.com/agent0ai/agent-zero-lite.git",
)
REPO_DIR = Path("/a0")
...
def launch_ui_process(repo_dir: Path, logger: AttemptLogger) -> subprocess.Popen[bytes]:
    ...
    logger.log("Starting Agent Zero Lite UI")
    ...
    *,
    ...
    try:
        ...
        if healthy:
            record_result(
                status="success",
                message=f"Updated Agent Zero Lite to branch {branch}, {resolved_target['target_description']}.",
...
def trigger_update_command(args: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Queue an Agent Zero Lite self-update for the next startup attempt.",
    )
    parser.add_argument(
        "branch",
        nargs="?",
    ...
    print("Queued Agent Zero Lite self-update for the next startup attempt.")
    ...
    print("Restart the container or Agent Zero Lite process to apply it.")
    return 0
